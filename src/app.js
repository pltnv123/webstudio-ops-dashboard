...
function production() {
  ...
  return `<div class="grid production-dashboard">
    ...
    ${card('v18 client-ready packages', rowsTop(asArray(progress.items).filter(item => item.phase === 'v18'), item => row(item.product_line, `${item.title || item.artifact_type} · ${ru(item.stage || 'stage')}`, item.status || 'artifact', `${shortPath(item.path)} · approval=${shortPath(item.qa_path || item.approval_path || '')} · готовность=${item.readiness_score ?? '—'}%`, 'artifact', jsonCopy(item)), 8, 'v18 артефакты пока не записаны'), 'span-12')}
