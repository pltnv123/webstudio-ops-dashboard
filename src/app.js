...
function productizationHero(marker, eyebrow, title, lede, links=[]) { return `<section class="productization-hero"><div class="productization-bar"><span>${fmt(marker)}</span><span>demo only</span><span>static sanitized snapshot</span></div><p class="productization-eyebrow">${fmt(eyebrow)}</p><h2>${fmt(title)}</h2><p class="productization-lede">${fmt(lede)}</p><div class="productization-actions">${links.map(l=>`<a class="productization-primary" href="${esc(l.href)}">${fmt(l.label)}</a>`).join('')}</div></section>`; }
