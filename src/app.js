...
function systemContinuationPanel(progress={}) {
  ...
  const body = `<p><b>${fmt(sys.title || 'System continuation layer')}</b></p>
    ...
    <div class="toolbar">${openButton('Manifest', sys.manifest_path || '')}${openButton('Operator runbook', sys.operator_runbook_path || '')}${openButton('Acceptance', sys.acceptance_path || '')}${copyButton('HERMES Auto-Push command', 'cd /home/hermes/workspace && WEBSTUDIO_STAGE=v19 bash /home/hermes/workspace/output/webstudio-github-autopush-v1.sh')}${detailPayloadButton(sys, 'Подробнее', 'system-v17')}</div>`;
  return card('System v19 — self-executing host pipeline', body, 'span-12 system-layer-card');
}
...
function production() {
  ...
  return `<div class="grid production-dashboard">
    ...
    ${card('v19 client delivery packets', rowsTop(asArray(progress.items).filter(item => item.phase === 'v19'), item => row(item.product_line, `${item.title || item.artifact_type} · ${ru(item.stage || 'stage')}`, item.status || 'artifact', `${shortPath(item.path)} · approval=${shortPath(item.qa_path || item.approval_path || '')} · готовность=${item.readiness_score ?? '—'}%`, 'artifact', jsonCopy(item)), 8, 'v19 артефакты пока не записаны'), 'span-12')}
